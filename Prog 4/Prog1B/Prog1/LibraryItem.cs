...
{
    public abstract class LibraryItem : IComparable<LibraryItem> //Implements IComparable Interface
    {
        ...
        // Library Item constructor: another item object supplied as an argument
        public LibraryItem(LibraryItem item) 
            : this(item.Title, item.Publisher, item.CopyrightYear, item.LoanPeriod, item.CallNumber) { }
        ...
        // Precondition:  None
        // Postcondition: When this < i2, method returns negative #
        //                When this == i2, method returns zero
        //                When this > i2, method returns positive #
        public int CompareTo(LibraryItem i2)
        {
            if (this == null && i2 == null) // Both null?
                return 0;

            if (this == null) // only this is null?
                return -1;

            if (i2 == null) // only i2 is null?
                return 1;
            
            int titleCompare = this.Title.CompareTo(i2.Title); //Compares the two titles 
