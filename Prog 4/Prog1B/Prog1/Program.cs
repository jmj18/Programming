...
public class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        //Gets Titles from the Library Item 
        var libraryTitle =
            from item in items
            where item is LibraryItem
            select item.Title;

        items.Sort(); // Sort - uses natural order
        Console.Out.WriteLine("Sorted list (Titles in Ascending Order):");
        foreach (String item in libraryTitle)
            Console.WriteLine(item);
        Pause();

        //Gets Copyright Year from Library Item
        var libraryYear =
            from item in items
            where item is LibraryItem
            select item.CopyrightYear;

        items.Sort(new YearCompare());
        Console.Out.WriteLine("Sorted list (Copyright Year in decending order) using Comparer:");
        foreach (int item in libraryYear)
            Console.WriteLine(item);
        Pause();
